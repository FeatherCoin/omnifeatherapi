...
def insertbtc(rawtx):
  try:
    ...
    propertyid = 0
    txtype = 0
    txversion = rawtx['FTC']['version']
    txhash = rawtx['FTC']['txid']
    ...
    dbExecute("insert into txjson (txdbserialnum, protocol, txdata) values (%s,%s,%s)", (txdbserialnum, protocol, json.dumps(rawtx['FTC'])) )

    dbCommit()
  except Exception,e:
    print_debug(("Error: ", e, "\n Could not add FTC PendingTx: ", rawtx),2)
    dbRollback()
